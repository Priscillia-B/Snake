...
# Si c'est la m�me direction ou direction interdite on sort
beq $a0 4 Exit
beq $a0 $t0 Exit
...
# Regarde si on demande pas une direction interdite (oppos�)
TestLegalBottom:
...
# Sauvegarde la nouvelle position entr�e par l'utilisateur
SetDirection:
sw $a0 snakeDir
...
# Permet d'acc�der a la derni�re adresse du tableau, un cran plus loin
lw $t5 tailleSnake
li $s1 4
mul $t5 $t5 $s1
add $t1 $t1 $t5
...
# D�cale tout le tableau Y vers la droite
UpY:
beq $t7 0 HeadUp
subi $t2 $t2 4
addi $t3 $t2 4
...
HeadUp: 
# Ajoute � la premi�re position (t�te), la nouvelle cordon�e + 1 en X
la $t1 snakePosX
lw $t3 4($t1)
addi $t3 $t3 1
sw $t3 0($t1)

# Ajoute � la premi�re position (t�te), la nouvelle cordon�e en Y qui est la m�me que la pr�c�dente
la $t2 snakePosY
lw $t3 4($t2)
sw $t3 0($t2)

# Acc�de au dernier �l�ment en dehors du tableau, pour remettre � 0 la cordonn�e que l'on a d�cal� en dehors du tableau
lw $t5 tailleSnake
li $s1 4
mul $t5 $t5 $s1
add $t1 $t1 $t5
...
# M�me chose que pour en bas et en haut
BougeADroite:
beq $t5 0 RightY
subi $t2 $t2 4
addi $t3 $t2 4
...
# M�me chose sauf que l� c'est l'inverse, c'est en Y qu'on va augmenter + 1
HeadRight: 
la $t2 snakePosY
lw $t3 4($t2)
addi $t3 $t3 1
...
# Acc�de au dernier �lement un cran plus loin
la $t1 snakePosX
la $t2 snakePosY
lw $t5 tailleSnake
li $s1 4
...
#M�me chose que pour bouger le serpent
CordoAddX:
beq $t5 0 ChangeYAdd
subi $t1 $t1 4
addi $t3 $t1 4
...
# R�cupere la t�te du serpent
lw $s3 0($s1) 
lw $s4 0($s2) 

# Comparaisons pour savoir si le serpent d�passe la limite

blt $s3 $t1 V0  #s'il d�passe � gauche
bgt $s4 $t2 V0 #s'il d�passe en haut
bgt $s3 $t2 V0  #s'il d�passe � droite
...
#Test si on sort pas du tableau, et si c'est le cas on test ensuite si le serpent se mange lui m�me, sinon on test en Y si �a correspond
TestObstacleX:
beq $t4 $t6 Suite
...
#Test si on sort pas du tableau, et si c'est le cas on test ensuite si le serpent se mange lui m�me, sinon on saute � V0 qui fait la fin du jeu
TestObstacleY:
beq $t5 $t6 Suite
beq $s4 $t3 V0
addi $t1 $t1 4
...
#M�me principe que pour les obstacles
TestEatLuiMemeX:
beq $t4 $t6 Fin
